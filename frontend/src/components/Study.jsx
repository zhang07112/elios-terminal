...
export default function Study({ api }) {
  ...
  return (
    <div className="memories-panel">
      <div className="memories-header">
        <div><h2>学习陪伴</h2><p>和Elios一起专注，番茄工作法</p></div>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16, padding: '32px 0', background: 'linear-gradient(135deg, rgba(124,191,154,0.08), rgba(148,201,115,0.08))', borderRadius: 24, marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 14, color: 'var(--color-text-muted)' }}>{isBreak ? '☕ 休息时间' : '📚 专注学习中'}</div>
        <div style={{ display: 'flex', gap: 12 }}>
          <button onClick={() => { if (running) { clearInterval(intv.current); setRunning(false) } else setRunning(true) }} className="send-btn" style={{ minWidth: 90 }}>{running ? '暂停' : '开始'}</button>
          <button onClick={() => { clearInterval(intv.current); setRunning(false); setTime(isBreak ? 5 * 60 : 25 * 60); setIsBreak(false); setMsg('') }} className="send-btn" style={{ background: 'var(--color-surface)', color: 'var(--color-text-primary)', boxShadow: 'none' }}>重置</button>
        </div>
      </div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <input value={topic} onChange={e => setTopic(e.target.value)} placeholder="今天学什么？" className="input-field" style={{ flex: 1 }} />
        <button onClick={() => { if (topic.trim()) { setTime(25 * 60); setRunning(true); setIsBreak(false); setMsg('') } }} disabled={!topic.trim()} className="send-btn">开始学习</button>
      </div>
      {msg && (
      ...
      {sessions.length > 0 && (
        <div>
          <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text-secondary)', marginBottom: 12 }}>📋 学习记录</div>
